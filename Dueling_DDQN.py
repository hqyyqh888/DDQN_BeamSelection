...
class Dueling_DDQN(DDQN):
    ...
    def __init__(self, config):
        ...
        self.q_network_local = self.create_NN(input_dim=self.state_size, output_dim=self.action_size + 1).to(self.device)
        
        #self.q_network_local.load_state_dict(torch.load('net_params_256_30dB_6res_block_no_sinr_reward.pkl'))
        
        self.q_network_optimizer = optim.Adam(self.q_network_local.parameters(), lr=self.hyperparameters["learning_rate"], eps=1e-4)
        self.q_network_target = self.create_NN(input_dim=self.state_size, output_dim=self.action_size + 1).to(self.device)
    ...
    def pick_action(self, state=None):
        ...
        if state is None: state = self.state.to(self.device)
        if len(state.shape) < 2: state = state.unsqueeze(0)
        self.q_network_local.eval()
        with torch.no_grad():
            action_values = self.q_network_local(state)
            if self.episode_step_number == 1:
                action_value_sorted,indices = torch.sort(action_values[:, :-1].detach()+action_values[:, -1].detach(),descending=True)
                self.action_value_temp += action_value_sorted.squeeze(0)
    ...
    def compute_expected_q_values(self, states, actions):
        """Computes the expected q_values we will use to create the lossclass 
